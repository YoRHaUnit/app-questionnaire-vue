  ...
  methods: {
    submitHandlerQuestion (event) {
      ...
      if (this.nbQuestions > parseInt(this.question)) {
        ...
        // Appel de la méthode stockant la question dans la bdd
        this.addDb()
    ...
    addDb(){
      // Crée une nouvelle base appelée "questionnaire"
      var db = new PouchDB('questionnaire')
      var date = new Date()
      // Insertion des données dans la base
      db.put({
        _id: this.firstname + date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate() + "-" + date.getHours() + "-" + date.getMinutes(),
        nom: this.firstname,
